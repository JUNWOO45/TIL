...
`null` 을 검사하는 것이 항상 더 낫다는 점은 당연하다. 

또한 벤치마크를 보면 알겠지만, `if문`과 `short-circuit 평가` 는 성능차이가 거의 없기때문에 더 마음에 드는 것을 사용하면 된다.
...
저는 두가지 케이스를 생각해보았습니다 : `filter` 메소드 사용 후 `map` 메소드를 사용하는 것과 `filter` 메소드 사용 후 `reduce` 메소드를 사용하는 케이스입니다.

각각의 케이스를 `메소드 체이닝`, `forEach`, `반복문` 을 사용하여 테스트해보았습니다.

차이점이 그리 크지 않다고 느껴질 수 있습니다.
...
`반복문` 이 가장 빠르긴 하지만 차이가 크진 않습니다.

`map` 에서는 `push` 를 하는 과정이 필요 없기 때문에 그 차이가 크지 않았습니다.
...
두 함수를 하나로 합쳤을 때, 거의 100%의 성능향상을 보여줍니다.(Reduce와 ForEach)

하지만, `반복문` 으로 변경하면 속도는 비교할 수 없을 정도로 향상되었습니다.
...
역시나 `기본 반복문` 이 다른 접근법보다 훨씬 빠른 것을 볼 수 있습니다.

그러나, 이것은 배열에 한정된 결과입니다.

다른 상황에서는 `forEach` 나 `for...of` 를 사용하는 것이 좋습니다.

하지만 `for...in`  반복문은 음..글쎄요. 다른 방법이 정녕 없는 경우를 제외하고는 사용하지 않는 것이 좋겠네요.

게가 `for...in` 반복문은 모든 객체의 프로퍼티(배열의 경우 인덱스)를 처리하기때문에 예상치못한 결과가 발생할 수 있습니다.
...
그러나, 몇몇 사람들은 DOM에서 엘리먼트를 가져와서 약간 수정하는 등의 간단한 작업을 위해 jQuery를 반드시 사용해야한다는 착각을 하곤 합니다.
...
당연하게도, 가장 기본적인 메소드 - `getElementById`와 `getElementsByClassName` - 이 DOM을 순회할 때 가장 뛰어난 성능을 보여주었습니다.

`querySelector by id` 와 `Advanced selector in querySelector` 또한 jQuery보다 빠른 성능을 보여주었습니다.

오직 `querySelectorAll` 의 경우에만 jQuery보다 느렸습니다.(클래스 명으로 엘리먼트들을 가져올 때)
...
하지만 간단한 DOM조작의 경우에도 라이브러리가 필요하다는 의미는 절대 아니라는 점이 중요합니다.
...
이 다섯가지 팁들은 자바스크립트로 더 빠른 코드를 작성하는데 도움이 되는 방법들이었습니다.

다른 방법들로 성능을 향상시키고 싶다면, 몇가지 체크해봐야할 사항들이 있습니다.

1. Webpack으로 자바스크립트 번들링을 최적화하는 방법에대해 알아보세요.
   이는 매우 광범위한 주제이지만, 제대로 수행한다면 당신의 어플리케이션의 로딩 시간이 매우 크게 향상될 수 있습니다.
2. 자료구조, 기본적인 알고리즘과 그 알고리즘의 복잡성에대해 공부해보세요.
   사람들은 이것들이 단지 이론적인 지식에 불과하다고 생각하지만, 우리는 이미 첫번째 팁에서 이 지식들이 어떻게 영향을 끼치는지 확인해봤습니다.
3. [jsPerf](https://jsperf.com/popular) 에서 테스트케이스들을 찾아보세요.
